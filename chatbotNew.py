...
pip.main(['install', 'tensorflow==0.12.1'])
...
def chatbot(net, sess, chars, vocab, max_length, beam_width, relevance, temperature):
    ...
    ctime = os.stat('userInput').st_ctime
    while True:
        if os.stat('userInput').st_ctime > ctime:
          userInput = open('userInput','r')
          bufferText = userInput.read()
          userInput.close()
          user_input = bufferText
          ...
          for i, char_token in enumerate(computer_response_generator):
              ...
              if i >= max_length: 
                break
          if onLoop >= sum(1 for chars in computer_response_generator):
            print("User Question: ",user_input, "\nDave Response: ", daveText)
            daveOut = open('daveOutput','w')
            daveOut.write(daveText)
            daveOut.close()
            daveText = "" 
